...
{
public:
    AbstractComputeEngine(std::shared_ptr<ComputeEngineInterface> computationManager, int id): computationManager(computationManager), id(id) {}
...
protected:
     ...
     */
    virtual ComputationType myType() const = 0;
     ...
     */
    virtual bool isComputationDone() const = 0;
     ...
     */
    virtual int getCurrentRequestId() const = 0;
        ...
        try {
            for(;;) {
                ...
                for(;;) {
                    ...
                    } else if (!computationManager->continueWork(getCurrentRequestId())) {
                        stopComputation();
                        break;
                    }
                }
...
protected:
    ...
    bool isComputationDone() const override {return computationDone;}
    double getResult() const override {return result;}
    int getCurrentRequestId() const override {return currentRequest.getId();}
...
{
public:
    ComputeEngineA(std::shared_ptr<ComputationManager> computationManager): AbstractComputeEngine(computationManager, nextId++) {}

protected:
    ComputationType myType() const override {return ComputationType::A;}
...
{
public:
    ComputeEngineB(std::shared_ptr<ComputationManager> computationManager): AbstractComputeEngine(computationManager, nextId++) {}

protected:
    ComputationType myType() const override {return ComputationType::B;}
...
// Computation engine B will be a simple divider
class ComputeEngineC : public ComputeEngineCommon
{
public:
    ComputeEngineC(std::shared_ptr<ComputationManager> computationManager): AbstractComputeEngine(computationManager, nextId++) {}
protected:
    ComputationType myType() const override {return ComputationType::C;}
